...
(** Momentum strategy signal generator.
    ...
    and relative volume gate.  Enabled at Small phase ($2,500+). *)
...
let generate ~symbol ~ema12 ~ema26 ~prev_ema12 ~prev_ema26
    ~sma50 ~sma200 ~rsi14 ~rel_volume ~price ~now =
  (* Volume gate: require RVOL >= 1.5 *)
  if rel_volume < 1.5 then None
  else
    ...
    (* RSI confirmation *)
    let rsi_oversold = rsi14 < 30.0 in
    let rsi_overbought = rsi14 > 70.0 in
    ...
    | None -> None
    | Some side ->
      (* Confidence: base 0.6, +0.1 for RSI confirmation, +0.1 for strong RVOL *)
      ...
      in
      let vol_bonus = if rel_volume >= 2.0 then 0.1 else 0.0 in
