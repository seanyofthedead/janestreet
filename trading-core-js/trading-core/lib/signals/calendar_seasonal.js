...
function combine(tom, sim, fomc) {
  const score_of = function (param) {
    switch (param) {
      case /* Bullish_window */ 0 :
        return 1.0;
      case /* Neutral */ 1 :
        return 0.0;
      case /* Bearish_window */ 2 :
        return -1.0;
    }
  };
  const total = score_of(tom) + score_of(sim) + 2.0 * score_of(fomc);
  if (total > 0.5) {
    return /* Bullish_window */ 0;
  } else if (total < -0.5) {
    return /* Bearish_window */ 2;
